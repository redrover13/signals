...
export function useSignal<T>(signal: Signal<T>): [T, (value: T | ((prev: T) => T)) => void] {
  ...
  useEffect(() => {
    // Only subscribe if it's not the initial mount
    if (isInitialMount.current) {
      isInitialMount.current = false;
      return; // Exit early without creating a subscription
    }
    
    // Subscribe to signal updates only after the initial mount
...
/**
 * Utility type to extract the value type from a Signal
 * @example
 * ```
 * const userSignal = createSignal({ name: 'John' });
 * type User = SignalValue<typeof userSignal>; // { name: string }
 * ```
 */
export type SignalValue<S> = S extends Signal<infer T> ? T : never;

/**
 * Utility type to unwrap potential signal types
 * Useful for functions that can accept either a signal or its raw value
 * @example
 * ```
 * function processUser(user: UnwrapSignal<typeof userSignal>) {
 *   // user is now treated as { name: string }
 * }
 * 
 * // Both work:
 * processUser(userSignal.get());
