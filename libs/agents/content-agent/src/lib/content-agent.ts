...
export class ContentAgent extends LlmAgent {
   ...
   */
  async createSocialContent(platform: string | undefined, restaurantInfo: any): Promise<any> {
    const prompt = `
Create social media content for platform: ${platform}

Restaurant Information: ${JSON && JSON.stringify(restaurantInfo, null, 2)}
...
Dishes: ${JSON && JSON.stringify(dishes, null, 2)}
