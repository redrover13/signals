...
export class DefaultRAGClient extends RAGClient {
  ...
  constructor(config?: {
    ...
    this.defaultDataStoreId = config?.dataStoreId || process.env.RAG_DATASTORE_ID || '';
    this.defaultSearchEngineId = config?.searchEngineId || process.env.RAG_SEARCH_ENGINE_ID || '';
  }
  ...
  async initialize(): Promise<void> {
    try {
      ...
      await this.vertexClient.initialize();
      
      // Verify data store access if configured
      if (this.defaultDataStoreId) {
        await this.vertexClient.verifyDataStoreAccess(this.defaultDataStoreId);
      }
      
      // Verify search engine access if configured
      if (this.defaultSearchEngineId) {
        await this.vertexClient.verifySearchEngineAccess(this.defaultSearchEngineId);
      }
      
      this.isInitialized = true;
    } catch (error) {
      throw errorHandler(error, {
        message: `Failed to initialize RAG client: ${error.message}`,
        category: GeminiErrorCategory.RAG_INTEGRATION_ERROR,
      });
    }
  }
  ...
  async searchDocuments(
    ...
    try {
      ...
      return await this.vertexClient.searchDocuments(query, searchOptions);
    } catch (error) {
      throw errorHandler(error, {
