...
export class MCPClientService {
  ...
  private setupHealthChecks(): void {
    try {
      if (this.config && config.global.healthMonitoring && this.config && config.global.healthMonitoring.enabled) {
      ...
      this.loggingService.error('Failed to setup health checks', error);
    }
  }
  ...
  private async connectToServer(config: MCPServerConfig): Promise<void> {
    try {
      ...
      if (healthCheck && healthCheck.status === 'healthy') {
        this.loggingService.log(LogLevel.INFO, `Successfully connected to server: ${config.name}`);
      } else {
        throw new MCPError(
          ErrorCodes.SERVER_UNHEALTHY,
          `Server ${config.name} is unhealthy: ${healthCheck && healthCheck.details || 'No details available'}`,
        );
      }
    } catch (error) {
      this.loggingService.error(`Failed to connect to server: ${config.name}`, error);
      throw error;
    }
  }
  ...
  async checkServerHealth(config: MCPServerConfig): Promise<HealthCheckResult | null> {
    try {
      ...
      const response = await firstValueFrom(
        this.httpService.get<HealthCheckResult>(url).pipe(
          catchError((error: AxiosError) => {
            this.loggingService.error(`Health check failed for server: ${config.name}`, error);
            return of({
              data: {
                status: 'unhealthy',
      ...
      this.loggingService.error(`Error during health check for server: ${config.name}`, error);
    ...
    for (const server of this.config.servers) {
      if (server.enabled) {
        try {
          ...
          this.loggingService.error(`Failed to check health for server: ${server.name}`, error);
        }
      }
    }
  ...
  async invokeServer(serverName: string, request: MCPRequest): Promise<MCPResponse> {
    const server = this.getServerConfig(serverName);
    if (!server) {
      throw new MCPError(ErrorCodes.SERVER_NOT_FOUND, `Server not found: ${serverName}`);
    }

    if (!server.enabled) {
      throw new MCPError(ErrorCodes.SERVER_DISABLED, `Server is disabled: ${serverName}`);
    }
  ...
  getServerConfig(serverName: string): MCPServerConfig | undefined {
    return this.config && config.servers && servers.find((s) => s.name === serverName);
  }
  ...
  getEnabledServers(): MCPServerConfig[] {
    return this.config && config.servers && servers.filter((server) => server.enabled) || [];
  }
}
