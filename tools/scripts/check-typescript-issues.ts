...
const checkTypeScriptIssues = (): void => {
  try {
    ...
    if (parsedConfig.errors.length) {
      throw new Error(
        `Error parsing tsconfig: ${parsedConfig.errors?.[0]?.messageText || 'Unknown error'}`,
      );
    }
    ...
    const issues: TypeScriptIssue[] = diagnostics.map((diagnostic) => {
      ...
      if (diagnostic.file) {
        ...
        const { line: lineNum, character: charNum } = diagnostic.file.getLineAndCharacterOfPosition(
          diagnostic.start!,
        );
    ...
    if (issues.length > 0) {
      ...
      issues.forEach((issue) => {
        console.log(
          `${issue.severity.toUpperCase()}: ${issue.filePath}:${issue.line}:${issue.character} - ${issue.message} (TS${issue.code})`,
        );
      });
