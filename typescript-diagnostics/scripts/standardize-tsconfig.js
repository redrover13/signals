...
function analyzeTsConfigs(tsconfigFiles) {
  ...
  console.log(
    `Found ${Object.keys(configAnalysis.inconsistentSettings).length} inconsistent settings`,
  );
}
...
function generateRecommendations() {
  ...
  const needsBaseConfig =
    configAnalysis.configs.some((c) => !c.hasExtendsRoot) &&
...
function saveReport() {
  ...
  fs.writeFileSync(path.join(REPORTS_DIR, 'tsconfig-analysis.md'), readableReport.join('\n'));
