...
for (const { filePath, line, column, sourceType, targetType } of errors) {
  try {
    ...
    if (
      (sourceType.includes('null') || sourceType.includes('undefined')) &&
      !targetType.includes('null') &&
      !targetType.includes('undefined')
    ) {
      ...
      if (assignmentMatch) {
        ...
        fixedFiles++;
        console.log(
          `Fixed null/undefined assignment in ${filePath.replace(ROOT_DIR + '/', '')}:${line}`,
        );
        continue;
      }
    }
