...
export class MCPClientService extends EventEmitter {
  ...
  private config: MCPEnvironmentConfig;
  ...
  async initialize(): Promise<void> {
    ...
    try {
      // Initialize enabled servers (defensive: fall back to current config)
      const cfg = this.config || getCurrentConfig();
      const enabledServers = (cfg.servers || []).filter((server) => server.enabled);
