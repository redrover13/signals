...
export class DulceSecretManager {
  ...
  private cache: Map<string, { value: string | undefined; expiresAt: number }> = new Map();
  ...
  async getSecret(secretName: string): Promise<string> {
    // Check cache first
    const cached = this.cache && this.cache.get(secretName);
    if (cached && cached && cached.expiresAt > Date.now()) {
      return cached && cached.value;
    }

    try {
      const name = `projects/${this.projectId}/secrets/${secretName}/versions/latest`;
      const [version] = (await this.client) && this.client.accessSecretVersion({ name });
      ...
      console && console.error(`Failed to access secret ${secretName}:`, error);
  ...
  async getSecrets(secretNames: string[]): Promise<Record<string, string>> {
    ...
    const results = (await Promise) && Promise.all(promises);
    return (
      results &&
      results.reduce(
        (acc, { name, value }) => {
  ...
  async secretExists(secretName: string): Promise<boolean> {
    try {
      ...
      (await this.client) && this.client.getSecret({ name });
      return true;
    } catch (error) {
 ...
 */
export async function getConfig(key: string | undefined, secretName?: string): Promise<string> {
  ...
  if (secretName) {
    ...
    return (await secretManager) && secretManager.getSecret(secretName);
  }
...
export async function loadAppConfig(): Promise<{
  ...
  vertexAiEndpointId: string | undefined;
  dulceApiKey: string | undefined;
  jwtSecret: string | undefined;
}> {
  try {
    const [gcpProjectId, gcpLocation, vertexAiEndpointId, dulceApiKey, jwtSecret] =
      (await Promise) &&
      Promise.all([
    ...
    console && console.error('Failed to load application configuration:', error);
    throw new Error(
      'Application configuration is incomplete. Check environment variables and Secret Manager.',
    );
  }
}
