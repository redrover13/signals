...
const ERROR_CATEGORIES = {
  STRICT_NULL_CHECKS: [
    ...
    { code: 'TS2533', pattern: /Object is possibly 'null' or 'undefined'/ },
    {
      code: 'TS2322',
      pattern:
        /Type '[^']+' is not assignable to type '[^']+'\.\s+Type '[^']+' is not assignable to type 'null'/,
    },
  ],
  TYPE_DECLARATION: [
...
function parseTypeScriptErrors(tscOutput) {
  ...
  // Output summary
  console.log(
    `Found ${diagnosticData.totalErrors} TypeScript errors in ${diagnosticData.totalFiles} files.`,
  );
...
function analyzeTypeScriptConfigs() {
  ...
  for (const configPath of tsconfigFiles) {
    try {
      ...
      const criticalOptions = ['moduleResolution', 'esModuleInterop', 'strict', 'target', 'module'];
      ...
      if (
        ...
        !['ESNext', 'ES2020', 'ES2022'].includes(compilerOptions.module)
      ) {
        configAnalysis.inconsistencies.push({
          config: relativeConfigPath,
...
function analyzeModuleSystem() {
  ...
  console.log(
    `Found ${moduleSystemAnalysis.inconsistencies.length} module system inconsistencies.`,
  );
...
function analyzeImportExports() {
  ...
  const esmExportRegex =
    /export\s+(?:default\s+)?(?:const|let|var|function|class|interface|type|enum|{)/g;
...
function analyzeErrorPatterns() {
  ...
  const commonPatterns = [
    ...
    },
    {
      ...
      recommendation:
        "Ensure your tsconfig.json has 'module' set to 'ESNext' and 'target' to at least 'ES2017'.",
    },
  ];
  ...
  if (
    Object.values(diagnosticData.errorCategories).some(
  ...
  console.log(
    `Generated ${diagnosticData.recommendations.length} recommendations based on error patterns.`,
  );
}
...
function generateSummaryReport() {
  ...
  // Save summary report
  fs.writeFileSync(path.join(REPORTS_DIR, 'summary-report.json'), JSON.stringify(summary, null, 2));
  ...
  summary.topErrorCategories.forEach((category) => {
    readableReport.push(
      `- TS${category.code}: ${category.message} (${category.count} occurrences)`,
    );
  });
  ...
  fs.writeFileSync(path.join(REPORTS_DIR, 'summary-report.md'), readableReport.join('\n'));
