...
export class ConnectionPoolService {
  ...
  async shutdown(): Promise<void> {
    ...
    await Promise.all(serverIds.map((serverId) => this.drainPool(serverId)));
  ...
  private async createConnection(
    ...
    try {
      ...
      throw new Error(
        `Failed to create connection to server ${serverId}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
   ...
   */
  private async createActualConnection(
    serverConfig: MCPServerConfig,
    timeout: number,
  ): Promise<unknown> {
  ...
  private cleanup(): void {
    ...
    for (const [serverId, pool] of this.pools.entries()) {
      ...
      if (this.idleTimes.length > 0) {
        stats.averageIdleTime =
          this.idleTimes.reduce((sum, time) => sum + time, 0) / this.idleTimes.length;
      }

      if (this.lifetimes.length > 0) {
        stats.averageLifetime =
          this.lifetimes.reduce((sum, time) => sum + time, 0) / this.lifetimes.length;
      }

      if (stats.totalAcquired > 0) {
        stats.acquireSuccessRate =
          (stats.totalAcquired / (stats.totalAcquired + stats.waitingRequests)) * 100;
      }
  ...
  private updateStats(
    serverId: string | undefined,
    statName: keyof ConnectionPoolStats,
    value: number,
  ): void {
    if (!serverId) return;
