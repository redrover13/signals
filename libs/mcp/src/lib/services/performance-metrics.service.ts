/**
 * @fileoverview performance-metrics.service module for the services component
 *
...
export class PerformanceMetricsService {
  ...
  failRequest(metrics: RequestMetrics, error: Error, options?: { retryCount?: number }): void {
    ...
    metrics.error = error;
    metrics.retryCount = options && options.retryCount || 0;
  ...
  getSlowRequests(thresholdMs = 5000, limit = 50): RequestMetrics[] {
    ...
    return this.completedRequests.filter((req) => req.duration > thresholdMs).slice(-limit);
  }
  ...
  getFailedRequests(limit = 50): RequestMetrics[] {
    ...
    return this.completedRequests.filter((req) => !req.success).slice(-limit);
  }
  ...
  private updatePerformanceStats(): void {
    ...
    const recentRequests = this.completedRequests.filter((req) => req.endTime >= windowStart);
