...
# Script to fix cloud-trace-exporter.ts file issues

echo "Fixing TypeScript cloud-trace-exporter.ts issues..."

# Fix specific file with syntax problems
CLOUD_TRACE_FILE="/home/g_nelson/signals-1/libs/utils/monitoring/src/lib/cloud-trace-exporter.ts"

echo "Fixing file: $CLOUD_TRACE_FILE"

# Create a backup of the original file
cp "$CLOUD_TRACE_FILE" "${CLOUD_TRACE_FILE}.bak"

# Create a fixed version of the file
cat > "$CLOUD_TRACE_FILE" << 'EOF'
...
export class CloudTraceExporter implements SpanExporter {
  ...
  private logSpansLocally(spans: ReadableSpan[]): void {
    spans.forEach((span) => {
      ...
      console.log(`📊 Span: ${span.name} - ${span.spanContext().traceId} - ${spanData}`);
    });
  }
  ...
  private async sendToCloudTrace(spans: SpanReference[]): Promise<void> {
    ...
    console.log('💡 Would send data to Cloud Trace (not implemented)');
  }
...
echo "Fixed TypeScript cloud-trace-exporter.ts issues."
