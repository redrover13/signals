...
const agents = {
  ...
  'content-agent': {
    ...
    async process(query: string, params?: Record<string, unknown>) {
      ...
      if (query.toLowerCase().includes('vietnamese content') || query.toLowerCase().includes('vietnamese localization')) {
  ...
  'gemini-orchestrator': {
    ...
    async process(query: string, params?: Record<string, unknown>) {
      ...
      if (queryLower.includes('content') || queryLower.includes('menu') || queryLower.includes('vietnamese')) {
...
export async function GET(
  ...
  try {
    ...
    const capabilities = {
      'bq-agent': ['SQL queries', 'F&B analytics', 'Vietnamese cuisine metrics', 'Cultural dining patterns'],
      'crm-agent': ['Customer management', 'Loyalty programs', 'Vietnamese preferences', 'Cultural celebrations'],
      'content-agent': ['Menu generation', 'Vietnamese localization', 'Cultural authenticity', 'Traditional recipes'],
      'looker-agent': ['Dashboard creation', 'Cultural analytics', 'Festival impact analysis', 'Authenticity metrics'],
      'reviews-agent': ['Sentiment analysis', 'Cultural feedback', 'Authenticity assessment', 'Vietnamese community insights'],
      'gemini-orchestrator': ['Task delegation', 'Cultural context routing', 'Multi-agent coordination', 'Vietnamese F&B optimization']
    };
