...
export class VertexAIClient {
  ...
  constructor(config: VertexAIClientConfig) {
    ...
    this.client = new PredictionServiceClient({
      // The client will automatically use ADC from the environment.
      // Client sẽ tự động sử dụng ADC từ môi trường (service account).
      apiEndpoint: `${location}-aiplatform.googleapis.com`,
    });

    this.endpoint = `projects/${project}/locations/${location}/endpoints/${config.endpointId}`;
  }
