...
export function createTool<TInput, TOutput>(
  ...
  return {
    ...
    toFunctionDeclaration: () => {
      ...
      const jsonSchema = inputSchema.innerType ? inputSchema.innerType() : {};
...
export const bigQueryQueryTool = createTool<BigQueryInput, { rows: unknown[] }>(
  'bq.query',
  'Run a BigQuery SQL query. Input: { sql: string, params?: object }',
  bigQueryInputSchema,
  async (input) => {
    try {
      ...
      throw errorHandler(error as Error, { sql: input.sql });
    }
  }
);
...
export const bigQueryInsertTool = createTool<BigQueryInsertInput, { ok: boolean }>(
  'bq.insert',
  'Insert rows into a BigQuery table. Input: { table: string, rows: any[] }',
  ...
  async (input) => {
    try {
      ...
      throw errorHandler(error as Error, { table: input.table, rowCount: input.rows.length });
    }
  }
);
...
export const storageUploadTool = createTool<StorageUploadInput, { uri: string }>(
  ...
  async (input) => {
    try {
      ...
      throw errorHandler(error as Error, { path: input.path });
    }
  }
);
 ...
 */
export const tools = {
  'bq.query': bigQueryQueryTool,
  'bq.insert': bigQueryInsertTool,
  'storage.uploadString': storageUploadTool,
} as const;
...
export function getToolFunctionDeclarations(): FunctionDeclaration[] {
  return Object.values(tools).map(tool => tool.toFunctionDeclaration());
}
