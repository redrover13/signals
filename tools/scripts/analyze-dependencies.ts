...
const analyzeDependencies = (): void => {
  try {
    ...
    projects.forEach((project) => {
      ...
      if (project.name) dfs(project.name);
    });
    ...
    projects.forEach((project) => {
      ...
      project.dependencies.forEach((dep) => {
        ...
        const depImplicitDeps =
          JSON.parse(fs.readFileSync(path.resolve(depProject.path || ''), 'utf8'))
      ...
      implicitDeps.forEach((implicitDep: string) => {
        const isUsed = projects.some(
          (p) =>
            ...
            (projectMap[implicitDep] &&
              projectMap[implicitDep].dependencies.includes(p.name || '')),
        );
