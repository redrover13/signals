...
function fixIssues(filePath) {
  ...
  if ((ext === '.ts' || ext === '.tsx') && !content.includes('@fileoverview')) {
    try {
      ...
      if (!filePath.includes('.spec.') && !filePath.includes('.test.') && !filePath.includes('jest.config') && !filePath.includes('tsconfig')) {
        console.log(`Adding standard header to ${filePath}`);
        
        const headerTemplate = fs.readFileSync(path.join(rootDir, '.github/templates/ts-header.template'), 'utf8');
