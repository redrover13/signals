...
console.log(`Scope: ${analyzeAll ? 'All projects' : (projectName ? `Project: ${projectName}` : 'Not specified (using affected)')}`);
...
function runDiagnosticsOnFiles(files, project) {
  ...
  try {
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(projectReportDir, 'typescript-errors.log'),
        tscOutput
      );
...
function generateSummaryReport(projects) {
  ...
  const projectsByErrors = [...summary.projects].sort((a, b) => b.typeScriptErrors - a.typeScriptErrors);
  const projectsWithLodashIssues = summary.projects.filter(p => p.lodashUsage.commonjs > 0 || p.lodashUsage.mixed > 0);
  ...
  if (projectsByErrors.length > 0 && projectsByErrors[0].typeScriptErrors > 0) {
    ...
    for (let i = 0; i < Math.min(3, projectsByErrors.length); i++) {
      if (projectsByErrors[i].typeScriptErrors > 0) {
        markdownSummary.push(`1. **${projectsByErrors[i].name}** (${projectsByErrors[i].typeScriptErrors} errors)`);
      }
    }
  ...
  if (projectsWithLodashIssues.length > 0) {
    ...
    for (const project of projectsWithLodashIssues) {
      markdownSummary.push(`- **${project.name}** (${project.lodashUsage.commonjs} CommonJS, ${project.lodashUsage.mixed} mixed imports)`);
    }
  ...
  markdownSummary.push('');
  markdownSummary.push('1. Run the TypeScript diagnostic tools with the `--fix` flag to automatically apply fixes');
  markdownSummary.push('2. Review project-specific reports in `typescript-diagnostics/reports/<project>/`');
  markdownSummary.push('3. Create focused PRs for high-priority projects');
  
  fs.writeFileSync(
    path.join(REPORTS_DIR, 'nx-projects-summary.md'),
