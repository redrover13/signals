...
describe('BQAgent', () => {
  ...
  it('should handle query execution errors gracefully', async () => {
    ...
    const result = await agent.executeQuery('SELECT 1 as test');
    expect(result).toHaveProperty('success');
    expect(typeof result.success).toBe('boolean');
  });

  it('should provide F&B analytics interface', async () => {
    const result = await agent.getFBAnalytics('test-restaurant');
    expect(result).toHaveProperty('success');
    expect(typeof result.success).toBe('boolean');
  });

  it('should provide customer insights interface', async () => {
    const result = await agent.getCustomerInsights('test-customer');
    expect(result).toHaveProperty('success');
    expect(typeof result.success).toBe('boolean');
  });

  it('should provide menu performance interface', async () => {
    const result = await agent.getMenuPerformance('test-restaurant');
    expect(result).toHaveProperty('success');
    expect(typeof result.success).toBe('boolean');
  });
});
