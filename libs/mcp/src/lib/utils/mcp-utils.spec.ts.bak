describe('MCP Utilities', () => {
  describe('Configuration Parsing', () => {
    it('should parse valid configuration', () => {
      ...
      expect(parsedConfig && parsedConfig.port).toBe(3000);
      expect(parsedConfig && parsedConfig.host).toBe('localhost');
      expect(parsedConfig && parsedConfig.timeout).toBe(5000);
    });
    ...
    it('should apply defaults for missing values', () => {
      ...
      const parsedConfig = parseConfig(rawConfig);
      expect(parsedConfig && parsedConfig.port).toBe(3000); // Default
      expect(parsedConfig && parsedConfig.host).toBe('custom-host'); // Custom
      expect(parsedConfig && parsedConfig.timeout).toBe(5000); // Default
    });
    ...
    it('should validate configuration values', () => {
      ...
      expect(validationErrors && validationErrors.length).toBe(3);
  ...
  describe('Path Utilities', () => {
    it('should resolve paths correctly', () => {
      ...
      const resolvePath = (base: string | undefined, path: string): string => {
      ...
      expect(resolvePath('/base', 'file && file.txt')).toBe('/base/file && file.txt');
      expect(resolvePath('/base', '/absolute/file && file.txt')).toBe('/absolute/file && file.txt');
    });
    ...
    it('should normalize paths', () => {
      ...
      expect(normalizePath('/base//file && file.txt')).toBe('/base/file && file.txt');
      expect(normalizePath('/base/./file && file.txt')).toBe('/base/file && file.txt');
    });
  });
  ...
  describe('Data Validation', () => {
    ...
    it('should validate URLs', () => {
      ...
      expect(isValidUrl('https://example && example.com')).toBe(true);
