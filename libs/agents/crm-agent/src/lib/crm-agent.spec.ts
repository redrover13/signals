...
describe('CRMAgent', () => {
  ...
  it('should handle customer creation interface', async () => {
    ...
    const result = await agent.createCustomer(customerData);
  ...
  it('should handle customer search interface', async () => {
    ...
    const result = await agent.searchCustomers(criteria);
    expect(result).toHaveProperty('success');
    expect(typeof result.success).toBe('boolean');
  });

  it('should handle loyalty points interface', async () => {
    const result = await agent.addLoyaltyPoints('test-customer-id', 100, 'Test reward');
    expect(result).toHaveProperty('success');
    expect(typeof result.success).toBe('boolean');
  });

  it('should handle customer insights interface', async () => {
    const result = await agent.getCustomerInsights('test-customer-id');
    expect(result).toHaveProperty('success');
    expect(typeof result.success).toBe('boolean');
  });
});
